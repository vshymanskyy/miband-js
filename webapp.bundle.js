...
function throwIfNotStringOrBuffer (val, prefix) {
  if (!Buffer$10.isBuffer(val) && typeof val !== 'string') {
...
function HashBase (blockSize) {
  ...
  this._block = Buffer$10.allocUnsafe(blockSize);
...
HashBase.prototype.update = function (data, encoding) {
  ...
  if (!Buffer$10.isBuffer(data)) data = Buffer$10.from(data, encoding);
...
function EVP_BytesToKey (password, salt, keyBits, ivLen) {
  if (!Buffer$11.isBuffer(password)) password = Buffer$11.from(password, 'binary');
  if (salt) {
    if (!Buffer$11.isBuffer(salt)) salt = Buffer$11.from(salt, 'binary');
    if (salt.length !== 8) throw new RangeError('salt should be Buffer with 8 byte length')
  }

  var keyLen = keyBits / 8;
  var key = Buffer$11.alloc(keyLen);
  var iv = Buffer$11.alloc(ivLen || 0);
  var tmp = Buffer$11.alloc(0);
...
Cipher.prototype._update = function (data) {
  ...
  return Buffer$12.concat(out)
};

var PADDING = Buffer$12.alloc(16, 0x10);
...
function Splitter () {
  this.cache = Buffer$12.allocUnsafe(0);
}

Splitter.prototype.add = function (data) {
  this.cache = Buffer$12.concat([this.cache, data]);
};
...
Splitter.prototype.flush = function () {
  ...
  var padBuff = Buffer$12.allocUnsafe(len);
  ...
  return Buffer$12.concat([this.cache, padBuff])
};
...
function createCipheriv (suite, password, iv) {
  ...
  if (typeof password === 'string') password = Buffer$12.from(password);
  if (password.length !== config.key / 8) throw new TypeError('invalid key length ' + password.length)

  if (typeof iv === 'string') iv = Buffer$12.from(iv);
...
function Splitter$1 () {
  this.cache = Buffer$13.allocUnsafe(0);
}

Splitter$1.prototype.add = function (data) {
  this.cache = Buffer$13.concat([this.cache, data]);
};
...
function createDecipheriv (suite, password, iv) {
  ...
  if (typeof iv === 'string') iv = Buffer$13.from(iv);
  if (config.mode !== 'GCM' && iv.length !== config.iv) throw new TypeError('invalid iv length ' + iv.length)

  if (typeof password === 'string') password = Buffer$13.from(password);
  if (password.length !== config.key / 8) throw new TypeError('invalid key length ' + password.length)
